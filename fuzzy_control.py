...
class FuzzyPlayer(Player):
    def __init__(self, racket: Racket, ball: Ball, board: Board):
        ...
        # Antecedent : Define input variables for the fuzzy control system 
        self.x_dist = fuzzcontrol.Antecedent (np.arange(-400, 401, 1),'x_dist')
        self.y_dist=fuzzcontrol.Antecedent(np.arange(-200,201,1),'y_dist')

        # Consequent : Define output variable for paddle movement (velocity)
        self.velocity = fuzzcontrol.Consequent(np.arange(-200, 201, 1), 'velocity')

        # Define membership functions for x_dist : horizontal distances 
        self.x_dist['left'] = fuzz.trimf(self.x_dist.universe, [-400, -400, 0])
        self.x_dist['center'] = fuzz.trimf(self.x_dist.universe, [-400, 0, 400])
        self.x_dist['right'] = fuzz.trimf(self.x_dist.universe, [0, 400, 400])

        # Define membership functions for y _dist : vertical distances
        self.y_dist['close']=fuzz.trimf(self.y_dist.universe,[-400,0,0])
        self.y_dist['far'] = fuzz.trimf(self.y_dist.universe, [0, 400, 400])

        # Define Membership functions for paddle movement : velocity
        self.velocity['fast_left'] = fuzz.trimf(self.velocity.universe, [-200, -200, -100])
        self.velocity['fast_right'] = fuzz.trimf(self.velocity.universe, [100, 200, 200])
        self.velocity['stop'] = fuzz.trimf(self.velocity.universe, [-100, 0, 100])
        self.velocity['slow_left'] = fuzz.trimf(self.velocity.universe, [-200, -100, 0])
        self.velocity['slow_right'] = fuzz.trimf(self.velocity.universe, [0, 100, 200])
        
        # Define fuzzy rules for paddle control 
        rule1= fuzzcontrol.Rule(self.x_dist['left'] &  self.y_dist['close'],self.velocity['slow_right'])
        rule2 = fuzzcontrol.Rule(self.x_dist['right'] & self.y_dist['close'], self.velocity['slow_left'])
        rule3= fuzzcontrol.Rule(self.x_dist['center'] & self.y_dist['far'],self.velocity['stop'])
        rule4 = fuzzcontrol.Rule(self.x_dist['left'] & self.y_dist['far'], self.velocity['fast_right'])
        rule5 = fuzzcontrol.Rule(self.x_dist['right'] & self.y_dist['far'], self.velocity['fast_left'])

        # Initialize the fuzzy control system
        self.racket_controller = ctrl.ControlSystem([rule1, rule2, rule3, rule4, rule5])
        self.racket_control_simulator = ctrl.ControlSystemSimulation(self.racket_controller)
        

    def act(self, x_diff: int, y_diff: int):
        # Calculate the velocity based on the current ball position
    ...
    def make_decision(self, x_diff: int, y_diff: int):
        # Input the current distances to the fuzzy control system
        self.racket_control_simulator.input['x_dist'] = x_diff
        self.racket_control_simulator.input['y_dist'] = y_diff
        
        # Execute the fuzzy control system
        self.racket_control_simulator.compute()
        
        # Return the defuzzified output : the velocity 
        return self.racket_control_simulator.output['velocity']
